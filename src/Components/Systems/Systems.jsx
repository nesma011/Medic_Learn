...
const Systems = () => {
  ...
  return (
    ...
    {loading ? (
        <p className="text-center mt-32 text-xl font-semibold text-green
        -700">Loading systems...</p>
      ) : error ? (
        <p className="text-center mt-32 text-xl font-semibold text-red-600">{error}</p>
      ) : (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-green-100 to-green
        -500 p-6">
          <motion.div
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {systems.length > 0 ? (
                systems.map((system) => (
                  <motion.div
                    key={system.id}
                    className="p-8 rounded-xl cursor-pointer transition-all shadow-md text-xl font-semibold flex flex-col items-center bg-gray-100 text-gray-700 hover:bg-green
